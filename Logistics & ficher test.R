...
new_df = data.frame(Gender , Purchased)
...
table1 <- table(gender = new_df$Gender , purchase = Purchased)
table1
...
table1 <- table(gender = new_df$Gender , purchase = new_df$Purchased , percent = "rows")
table1
...
table1 <- table(gender = new_df$Gender , purchase = new_df$Purchased)
table1
...
dat <- data.frame(
  "Male" = c(130, 66),
  "Females" = c(127, 77),
  row.names = c( "Not Buying" , "Buying"),
  stringsAsFactors = FALSE
)
colnames(dat) <- c("Males", "Females")

dat



mosaicplot(new_df$Gender ~ new_df$Purchased,
           main = "Mosaic plot"
           )
